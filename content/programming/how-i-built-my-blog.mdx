...
It's been years since I typed my first line of code. I don't remember exactly what made me interested in learning to code, but I remember for sure the feeling when the first time I saw the output — `"Hello, World!"{:js}` — of my first line of code. As far as I remember, I wrote my first line of code in PHP.
...
This was the most explorative and confusing part of my blog. My blog is just a [NextJS](https://nextjs.org/) application at its core. I write my content in MDX and push it a Github repository to post it. No database is involved to store the content — just static mdx files.
...
![18×2 Beyond Youthful Days' IMDB page](/content/programming/how-i-made-this-blog/image-1.png "18x2 Beyond Youthful Days' IMDB page")
...
To fetch top tracks data from [Spotify API](https://developer.spotify.com/documentation/web-api), you just need to make a request to the `https://api.spotify.com/v1/me/top/tracks{:shell}` endpoint. The available optional query parameters for this endpoint are:

1. `time_range{:shell}`: Over what time frame the affinities are computed. Valid values: `long_term{:shell}` (calculated from ~1 year of data and including all new data as it becomes available), `medium_term{:shell}` (approximately last 6 months), `short_term{:shell}` (approximately last 4 weeks). Default: `medium_term{:shell}`
2. `limit{:shell}`: The maximum number of items to return. Default: `20{:shell}`. Minimum: `1{:shell}`. Maximum: `50{:shell}`.
3. `offset{:shell}`: The index of the first item to return. Default: `0{:shell}` (the first item).
...
The way this updates is honestly problematic because it's not precise. It will be updated on the first request after the `revalidate{:tsx}` interval (every 24h). Because of this, the cache might revalidate at unexpected times if the page is rarely visited.
